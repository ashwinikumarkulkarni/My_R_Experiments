...
## Individual genotype
#### WT-CTL
load("CTL_CB_DF_CLUST.RData")
wt.ctl.seurat <- seuObj
rm(seuObj)

## CKO-CTL
load("CKO_CB_DF_CLUST.RData")
cko.ctl.seurat.temp <- seuObj
rm(seuObj)

## CKO-RES
load("RES_CB_DF_CLUST.RData")
...
ggsave(filename = "SEURAT_PCA_1.pdf", plot = p6, width = 6, height = 4, units = "in", dpi = 150)
ggsave(filename = "SEURAT_PCA_2.pdf", plot = p7, width = 12, height = 6, units = "in", dpi = 150)
...
##-------------------------------------------------------
## SAVE RDATA
save(seuObj, file = "SEURAT_CTL_CKO_RES_HARMONY.RData")
...
ggsave(filename = "SEURAT_HARMONY_CLUSTREE.pdf", plot = seutree, width = 24, height = 24, units = "in", dpi = 150)
...
for (Resolution in ResolutionList)
    ...
    pdf(paste0("SEURAT_HARMONY_UMAP_RES_", Resolution, ".pdf"), width=7, height=6)
    g <- DimPlot(object = seuObj, label = TRUE, reduction = "umap", group.by = Resolution)
    print(g)
    dev.off()

    pdf(paste0("SEURAT_HARMONY_VIOLIN_nUMI_RES_", Resolution, ".pdf"), width=7, height=3)
...
ggsave(filename = "SEURAT_PLOT_FeaturePlot_orderT.pdf", plot = fpl1, width = 15, height = 24, units = "in", dpi = 150)
...
ggsave(filename = "SEURAT_PLOT_FeaturePlot_orderF.pdf", plot = fpl2, width = 15, height = 24, units = "in", dpi = 150)


plotCluUMAP1 <- DimPlot_scCustom(seurat_object = seuObj, group.by = "Genotype", pt.size = 0.1, reduction = "umap", label = FALSE, raster = TRUE)
ggsave(filename = "SEURAT_UMAP_GENOTYPE.pdf", plot = plotCluUMAP1, width = 10, height = 8, units = "in", dpi = 300)
...
ggsave(filename = "SEURAT_UMAP_GENOTYPE_FACET.pdf", plot = plotCluUMAP1b, width = 32, height = 8, units = "in", dpi = 300)
...
ggsave(filename = "SEURAT_UMAP_GENOVIRAGESAMPLE_FACET.pdf", plot = plotCluUMAP1c, width = 25, height = 20, units = "in", dpi = 300)

plotCluUMAP1d <- DimPlot_scCustom(seurat_object = seuObj, group.by = "GenoVirAgeSample", pt.size = 0.1, reduction = "umap", raster = TRUE)
ggsave(filename = "SEURAT_UMAP_GENOVIRAGESAMPLE.pdf", plot = plotCluUMAP1d, width = 8, height = 6, units = "in", dpi = 300)

plotCluUMAP1e <- DimPlot(seuObj, group.by = c("Sex", "Batch", "Genotype", "Virus", "GenoVirAgeSample"), pt.size = 0.01)
ggsave("SEURAT_UMAP_COVARIATES.pdf", plot = plotCluUMAP1e, width = 20, height = 16, units = "in", dpi = 300)
...
ggsave(filename = "SEURAT_PLOT_ViolinPlot_1.2_A.pdf", plot = vpl2 , width = 12, height = 18, units = "in", dpi = 150, useDingbats=FALSE)
...
ggsave(filename = "SEURAT_PLOT_ViolinPlot_1.2_B.pdf", plot = vpl3 , width = 12, height = 3, units = "in", dpi = 150, useDingbats=FALSE)
...
ggsave(filename = "SEURAT_PLOT_DotPlot_1.2.pdf", plot = dpl2, width = 12, height = 16, units = "in", dpi = 150, useDingbats=FALSE)
...
write.table(cellsPerCluster, paste("SEURAT_GENOTYPE_PER_CLUSTER_1.2.txt", sep = "_"), row.names = F, col.names = T, quote = F, sep = "\t")
...
p7 <- ggplot(cellsPerCluster2) +
        ...
        NULL
ggsave(filename = paste("SEURAT_GENOTYPE_PER_CLUSTER_1.2.pdf", sep = "_"), plot = p7, width = 9, height = 3, units = "in", dpi = 150)
...
cellsPerCluster$Cluster <- paste("Cluster", sprintf("%02d", as.numeric(row.names(cellsPerCluster))), sep = "_")
cellsPerCluster <- cellsPerCluster[order(cellsPerCluster$Cluster),]
write.table(cellsPerCluster, paste("SEURAT_GENOVIRAGESAMPLE_PER_CLUSTER_1.2.txt", sep = "_"), row.names = F, col.names = T, quote = F, sep = "\t")
...
p7 <- ggplot(cellsPerCluster2) +
        ...
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
        NULL
ggsave(filename = paste("SEURAT_GENOVIRAGESAMPLE_PER_CLUSTER_1.2.pdf", sep = "_"), plot = p7, width = 9, height = 3, units = "in", dpi = 150)
...
load("SEURAT_CTL_CKO_RES_HARMONY.RData")

Idents(seuObj) <- "RNA_snn_res.1.2"

seuObj.markers <- FindAllMarkers(seuObj, only.pos = TRUE) #, min.pct = 0, logfc.threshold = 0)
write.table(seuObj.markers, "SEURAT_CTL_CKO_RES_HARMONY_RES_1.2_CLUSTER_MARKERS.txt", row.names = TRUE, col.names = TRUE, quote = FALSE, sep = "\t")
save(seuObj.markers, file = "SEURAT_CTL_CKO_RES_HARMONY_RES_1.2_CLUSTER_MARKERS.RData")
